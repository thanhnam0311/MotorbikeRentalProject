...
public class HopDongDAO {
    ...
    public ArrayList<HopDongDTO> layHopDongTheoUser(int userId) {
        ...
        String sql = "SELECT r.contract_code, CONCAT(v.brand, ' ', v.model) AS vehicle_name, " +
                     ...
                     "JOIN CUSTOMERS c ON r.customer_id = c.customer_id " +
                     "JOIN VEHICLES v ON r.vehicle_id = v.vehicle_id " +
                     "WHERE c.user_id = ? ORDER BY r.created_at DESC";
    ...
    public boolean taoYeuCauThue(HopDongDTO hd) {
        // Lấy customer_id dựa vào user_id, mặc định created_by = 1 (Admin)
        String sql = "INSERT INTO RENTAL_CONTRACTS (contract_code, customer_id, vehicle_id, created_by, " +
                     "rental_start, rental_end, rental_type, deposit_amount, total_amount, contract_status) " +
        ...
        try (Connection conn = MySQLConnect.getConnection();
            ...
            ps.setString(6, hd.getRentalType());
            ps.setDouble(7, hd.getDepositAmount());
            ps.setDouble(8, hd.getTotalAmount());
